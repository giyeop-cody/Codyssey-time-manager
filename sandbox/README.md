...
## 부록 12: 알람 생애주기 정리 + "해제 전까지 최대 1분 울림" (25차 — v1.5.0)

### 알람 자동 삭제 경로 전수 (사용자 의혹 "울리기 전에 지워진다" 점검 결과)
| 경로 | 무엇을 지우나 | OS 알람도 취소? |
|---|---|---|
| getStoredAlarms() JS 자가정비 (1분 GET_ALARMS 등) | 지난 시각 항목을 **목록에서만** 제거 | ❌ 아니오 (OS 예약은 그대로 → 발화됨) |
| M6 AlarmReceiver.removeFiredAlarm | 발화된 알람을 목록에서 제거 (발화 '후') | 이미 발화됨 |
| K8 BootReceiver | 부팅 시점의 과거 항목 복원 생략+목록 정리 | 과거 항목만 |
| LOGOUT | 전체 알람 취소 + 목록 비움 | ✅ 예 (수동 로그아웃 시에만) |
| 세션 무효 확정(19차 clearSessionIdentity) | 쿠키 백업/member_id 폐기 — **알람 목록은 건드리지 않음** | ❌ |

→ 사전 삭제로는 "목록 증적 소실(혼동)"이 확인됐을 뿐, OS 예약 자체를 앞당겨 지우는 경로는 없음.
  무음 소멸의 실제 용의자는 23차가 다룬 클래스(PI 해시 충돌/권한/채널 차단)+제조사 지연이며, 계측은 ALARM-S/F·NOTIF로 판독.

### 25차: 울림 UX 변경 (사용자 요청)
- AlarmSoundService(전경, mediaPlayback): 발화 시 알람음(USAGE_ALARM, 루프) **최대 60초 반복** 재생.
- 고정 알림(채널 codyssey_alarms_ring, 소리 없음): 끄기 액션 + 탭(앱 열기)으로 해제.
- 매 60초 타임아웃: 울림 중지 + 울림 알림 제거 + DiagLog 기록 ("1분간 울렸다가 지워짐").
- alarm_sound=false 설정 시 기존 조용한 단발 알림 경로 유지.
- 목록 자가정비 유예 15분 추가 (발화 직후 즉시 목록 소멸로 보이던 혼동 방지; 부팅 경로는 자체 K8 로직 유지).
- 단발 채널 알림도 병행 유지(울림을 끈 뒤에도 기록은 남음).

## 부록 13: 퇴실/목표 알람 "계산 즉시 자동 등록" + 플랫폼 통일 원칙 (26차 — v1.6.0)

### 배경
기존: 계산 결과가 나와도 "설정" 버튼을 눌러야 알람이 잡힘 (누락 잦음).
변경: 계산되면 즉시 자동 등록, 사용자는 원치 않을 때 '해제'/목록 삭제만 수행.

### 규칙 (익스텐션/앱 공유 popup.js에서 구현 — 양 플랫폼 동일 동작)
1. 계산으로 시각이 결정되면 동일 시각·타입 미래 알람이 없을 때 자동 SET_ALARM 등록 + 버튼은 '해제' 상태.
2. 사용자가 어떤 경로(버튼/목록)로 해제해도 그 날은 자동 재등록 금지 (localStorage 당일 플래그).
   재등록은 '설정' 버튼의 수동 경로(플래그 해제).
3. 계산 시각이 바뀌면 오늘 날짜의 같은 타입 알람을 교체 (데이터 갱신 추종).
4. 자동 등록 시 알림: "HH:mm에 XX이 울립니다. (목록에서 삭제 가능)" — 정확 알람 권한 미허용 시 안내.
5. 평가 알람(E2/E3)은 이미 자동 등록(서버 목록 추종) — 변경 없음.

### 플랫폼 통일 원칙 (공유 자산)
- UI/계산/알람 정책은 `popup.js` + `shared-attendance.js` 단일 소스. 각 플랫폼은 어댑터(메시지→네이티브/크롬)만 담당.
- 의도적 차이(플랫폼 특성, 통일 불가): 앱=FGS 1분 감지/울림 서비스·절전 예외, 익스텐션=chrome.alarms+SW 주기. 문서상 기대값(알림 문구·예약 규칙·목록 규칙)은 동일.
