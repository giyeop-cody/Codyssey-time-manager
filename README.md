...
### 만든 이유
코디세이는 **월 필수 출입 시간**(기본 80시간)이 있고 서버는 **하루 최대 12시간**까지만 출입으로 인정한다.
그런데 실제로 쓰다 볼수록 이런 불편이 반복된다:

1. **출입 중에는 실시간으로 시간이 반영되지 않는다** — 오늘 얼마나 채웠는지 모른다.
2. **퇴실 시간을 언제로 할지 정확히 알기 힘들다** — 몇 시에 나가야 오늘/월 목표를 채우는지 매번 머리로 계산해야 한다.
3. **입·퇴실이 잘 찍혔는지 확인하려면 번거롭고, 누락이 있을 때가 있다** — 뒤늦게 발견하면 그 시간이 통째로 날아간다.

그리고 뒤집어 볼 때 — **남은 시간을 기준으로 당일 일정을 조정하고 스케줄링할 수 있으면 좋다**.
이 네 번째가 이 앱의 방향이다: 출입 시간은 "기록"이 아니라 "오늘 계획의 재료".

### 문제 → 기능 매핑
| 만든 이유 | 해결 기능 |
|---|---|
| ① 오늘 얼마나 채웠는지 모름 (실시간 반영 없음) | 대시보드: 입실~현재 경과를 합친 **오늘 인정 실시간 표시**·월 누적/잔여 진행률, 미니 캘린더(오늘=실시간 값), 5분 틱·자동 갱신 |
| ② 퇴실 타이밍 계산 어려움 | **계산기**: 퇴실 예정 시간→총 인정(일/월 잔여·12h 초과 경고), 목표 시간→예상 퇴실 시각 즉답 + 알람 자동 등록 |
| ③ 입·퇴실 확인 번거로움, 누락 | **게이트 감지**("입실/퇴실 처리됨" 알림), **자정 롤오버**(밤샘/퇴실 누락 확인), **물리 탐지(베타)** S1·S2 누락 의심 알림 |
| ④ 남은 시간 기준으로 일정 조정·스케줄링 | 일/월 **잔여 시간 상시 표시** + 계산기 예상 퇴실 시각 + **평가 일정 자동 연동**(N분 전 알람)으로 당일 계획 수립 지원 |
...
- **월 페이스/마감 경고**(36차): (월 필수 − 누적) ÷ 남은 날(오늘 포함) ≥ 12시간/일이면 🚨 마감 임박 — 매일 상한을 채워도 불가. 그 이틀 전부터는 ⏳ 페이스 경고(하루 1회, 같은 날 상위 레벨만 재알림). `monthlyDeadlineAlert`(JS) ↔ `MonthlyDeadlineCheck.java`(네이티브, 6시간 스로틀) 동일 규칙.
...
- **수집 모드**: 틱 샘플(시각·신호·세션·판정·활동)을 폰 내부 링버퍼(최대 1200건)에만 저장 → "내보낼 때만" FileProvider 공유 + **자동 전송 알림**(36차): 매일 1회·버퍼(1200건) 도달 시 알림 — 탭하면 첨부 완료된 메일 작성 화면이 열림(수신 이메일 미리 설정 가능). 백그라운드에서 메일 앱을 직접 열 수 없어 탭 경유. (자세한 건 `docs/beta-phy-collection.md`)
...
| 마감 경고 기준 | 일 필요 ≥ 12h → 🚨 / 전전날 ≥ 12h → ⏳ | monthlyDeadlineAlert ↔ MonthlyDeadlineCheck |
...
- **물리 탐지(베타)**: 근처 감지 · 수집 모드(기본 켬, OFF 가능) · 지오펜스 · 지금 위치 학습 · 수집 데이터 내보내기(공유 시트로 JSON 전달) · 수신 이메일(자동 전송 알림의 메일 받는 사람)
