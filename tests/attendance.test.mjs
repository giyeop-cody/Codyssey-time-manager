...
// ===== 45차: 날짜 상세 (캘린더 모달) =====
const dayDetailParsed = {
  dailyBreakdown: { '2026-07-22': 540 },
  rawDetailList: [
    { date: '2026-07-22', daily_total_duration: '09:00:00', sessions: [
      { entry_time: '09:05:00', exit_time: '18:05:00', is_missing: false, missing_type: null }
    ] },
    { date: '2026-07-24', daily_total_duration: '03:30:00', sessions: [
...
test('45차 getDaySessions: 날짜별 세션 반환 / 없으면 빈 배열', () => {
  assert.equal(getDaySessions(dayDetailParsed, '2026-07-22').length, 1);
  assert.equal(getDaySessions(dayDetailParsed, '2026-07-24').length, 2);
  assert.deepEqual(getDaySessions(dayDetailParsed, '2026-07-31'), []);
  assert.deepEqual(getDaySessions(null, '2026-07-22'), []);
  assert.deepEqual(getDaySessions({}, '2026-07-22'), []);
});

test('45차 describeDaySession: 정상 쌍 세션의 소요 시간', () => {
  const s = { entry_time: '09:05:00', exit_time: '18:05:00', is_missing: false, missing_type: null };
  const d = describeDaySession(s, '2026-07-22', '2026-07-24');
  assert.equal(d.kind, 'normal');
  assert.equal(d.minutes, 540);
});

test('45차 describeDaySession: 오늘 유효 개방 세션은 open', () => {
  const s = { entry_time: '13:05:00', exit_time: null, is_missing: true, missing_type: 'exit' };
  const ts = new Date('2026-07-24T14:00:00').getTime();
  assert.equal(describeDaySession(s, '2026-07-24', '2026-07-24', ts).kind, 'open');
});

test('45차 describeDaySession: 과거·13시간 초과 미퇴실은 stale_exit', () => {
  const s = { entry_time: '10:03:00', exit_time: null, is_missing: true, missing_type: 'exit' };
  assert.equal(describeDaySession(s, '2026-07-23', '2026-07-24').kind, 'stale_exit');
  const late = new Date('2026-07-24T23:30:00').getTime();
  assert.equal(describeDaySession(s, '2026-07-24', '2026-07-24', late).kind, 'stale_exit');
});

test('45차 describeDaySession: 입실 누락·역전 시각 분류', () => {
  assert.equal(
    describeDaySession({ entry_time: null, exit_time: '18:00:00', is_missing: true, missing_type: 'entry' }, '2026-07-22', '2026-07-24').kind,
    'no_entry');
